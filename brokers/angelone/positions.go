...
func (a *Angelone) GetPositions() (*model.Response[[]model.PositionResponse], error) {
	...
	for _, item := range items {
		out = append(out, model.PositionResponse{
			...
			NetQty:        util.ParseInt32(item.NetQty),
			LTP:           util.ParseFloat64(item.LTP),
			Close:         util.ParseFloat64(item.Close),
			Pnl:           util.ParseFloat64(item.Pnl),
			Realised:      util.ParseFloat64(item.Realised),
			Unrealised:    util.ParseFloat64(item.Unrealised),
