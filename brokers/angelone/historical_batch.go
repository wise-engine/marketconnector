...
func splitDateRangeIntoBatches(req model.HistoricalBatchRequest) ([]symbolBatch, error) {
	...
	from, err := time.Parse(util.DateFormat, req.FromDate)
	if err != nil {
		return nil, fmt.Errorf("invalid fromDate %q: %w", req.FromDate, err)
	}
	to, err := time.Parse(util.DateFormat, req.ToDate)
	if err != nil {
...
func (a *Angelone) FetchHistoricalDataBatch(requests []model.HistoricalBatchRequest) (*model.Response[[]model.HistoricalBatchItem], error) {
	if len(requests) == 0 {
